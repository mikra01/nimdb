...
proc hex2Str*(par : var openArray[byte]) : string =
  ...
  for i in countup(0,par.len-1):
    result[ 2 + cast[int](i shl 1) ] = hChars[cast[int](par[i] shr 4 )]    # process hs nibble
                                       # fetch index to get the hex-char out of the value
    result[ 3 + cast[int](i shl 1) ] = hChars[cast[int](par[i] and 0xF)] # process ls nibble
